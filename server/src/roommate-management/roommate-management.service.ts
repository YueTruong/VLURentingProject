...
export class RoommateManagementService {
  ...
  private async ensureRoommateCategory() {
    ...
    if (existingCategory) {
      if (
        ...
        'Danh mục bài đăng tìm người ở ghép'
      ) {
  ...
  private async publishApprovedRequest(request: RoommateRequestEntity) {
    ...
    const payload: Partial<PostEntity> = {
      ...
      price: Number.isFinite(roommatePrice) && roommatePrice > 0
        ? roommatePrice
  ...
  private async syncApprovedRequestPublicPost(
    request: RoommateRequestEntity,
  ) {
  ...
  async getListingOptions() {
    ...
    return posts.map((post) => ({
      ...
      landlordName: post.user?.profile?.full_name ?? post.user?.username ?? null,
      currentOccupancy: 0,
  ...
  async create(
    ...
    const roommateRequest = this.roommateRequestRepository.create({
      ...
      landlordName:
        listingPost.user?.profile?.full_name ?? listingPost.user?.username ?? null,
      currentOccupancy,
      maxOccupancy,
    });
