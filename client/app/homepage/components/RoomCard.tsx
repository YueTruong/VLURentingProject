...
export default function RoomCard({ data, className }: RoomProps) {
  ...
  const handleToggleFavorite = () => {
    ...
    if (!session) {
      toast.error("Vui lòng đăng nhập để yêu thích tin!", {
        style: {
          borderRadius: '10px',
          background: '#333',
          color: '#fff',
    ...
    if (isSaved) {
      toast("Đã bỏ yêu thích tin", { icon: '💔' });
    } else {
      toast.success("Đã yêu thích tin thành công!");
    }
  };

  return (
    <div
      className={`group relative flex h-full w-full max-w-[360px] flex-none flex-col overflow-hidden rounded-[20px] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_36px_rgba(0,0,0,0.14)] ${className ?? ""}`}
    >
      ...
      <div className="flex flex-col gap-4 p-5">
        <div className="space-y-1">
          <h3 className="line-clamp-2 text-[18px] font-semibold text-gray-900" title={data.title}>
            {data.title}
          </h3>
        ...
        <div className="flex items-center justify-between">
          <p className="text-[18px] font-bold text-red-500">
            {data.price} <span className="text-sm font-normal text-gray-500">/ tháng</span>
          </p>
          <Link
            href={`/listings/${data.id}`}
            className="rounded-[10px] bg-blue-50 px-4 py-1.5 text-sm font-medium text-blue-600 transition-all duration-300 hover:bg-blue-600 hover:text-white active:scale-95"
          >
            Xem ngay
          </Link>
        </div>
